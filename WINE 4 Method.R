...
X <- scale(as.matrix(wine[,2:14])); y <- as.integer(wine[,1])
k <- 3; L <- 8; n <- nrow(X)

km <- kmeans(X, centers=k, nstart=25)
kmm <- kmeans_minus(X,k,L); det <- rep(FALSE,n); det[kmm$outlier_idx] <- TRUE
# wine DBSCAN needs PCA rescue (raw 13D fails)
Xp <- prcomp(X)$x[,1:3]; bd <- tune_dbscan(Xp)
gmm <- Mclust(X, G=k, verbose=FALSE)

cat("============================================================\n")
cat("WINE - four-method clustering (k =", k, ")\n")
cat("============================================================\n")
